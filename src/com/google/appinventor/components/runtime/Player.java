    ...
    implements Component, android.media.MediaPlayer.OnCompletionListener, OnPauseListener, OnResumeListener, OnDestroyListener, OnStopListener, Deleteable
{
    ...
    public Player(ComponentContainer componentcontainer)
    {
        ...
        {
            audiomanager = FroyoUtil.setAudioManager(activity);
        } else
        {
            audiomanager = null;
        }
        am = audiomanager;
        flag = audioFocusSupported;
        obj = null;
        if (flag)
        {
            obj = FroyoUtil.setAudioFocusChangeListener(this);
        }
        afChangeListener = obj;
    }
    ...
    private void prepare()
    {
        ...
        aobj[0] = sourcePath;
        form.dispatchErrorOccurredEvent(this, "Source", 702, aobj);
    }
    ...
    private void requestPermanentFocus()
    {
        ...
        if (!focusOn)
        {
            ...
            Object aobj[] = new Object[1];
            aobj[0] = sourcePath;
            form.dispatchErrorOccurredEvent(this, "Source", 709, aobj);
        }
    }
    ...
    public void Source(String s)
    {
        ...
        if (sourcePath.length() > 0)
        {
            ...
            try
            {
                MediaUtil.loadMediaPlayer(player, this.form, sourcePath);
            }
            catch (IOException ioexception)
            {
                player.release();
