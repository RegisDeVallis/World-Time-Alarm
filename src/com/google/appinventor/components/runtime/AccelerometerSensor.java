    ...
    implements OnStopListener, OnResumeListener, SensorComponent, SensorEventListener, Deleteable
{
    ...
    private boolean isShaking(Queue queue, float f)
    {
        ...
        f2 = f1 / (float)queue.size();
        if (Sensitivity() != flag) goto _L2; else goto _L1
_L1:
        int i;
        if ((double)Math.abs(f2 - f) > 20D)
        {
            i = ((flag) ? 1 : 0);
        } else
        {
...
_L2:
        ...
        if ((double)Math.abs(f2 - f) > 13D && (double)Math.abs(f2 - f) < 20D) goto _L4; else goto _L3
_L3:
        return false;
        if ((double)Math.abs(f2 - f) > 5D && (double)Math.abs(f2 - f) < 13D) goto _L4; else goto _L5
_L5:
        return false;
    }
    ...
    public void AccelerationChanged(float f, float f1, float f2)
    {
        ...
        EventDispatcher.dispatchEvent(this, "AccelerationChanged", aobj);
    }
    ...
    public void Sensitivity(int i)
    {
        ...
        } else
        {
            ...
            form.dispatchErrorOccurredEvent(this, "Sensitivity", 1901, aobj);
            return;
        }
    }
