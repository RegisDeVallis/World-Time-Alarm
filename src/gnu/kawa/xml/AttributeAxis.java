...
public class AttributeAxis extends TreeScanner
{
    ...
    {
        int j = abstractsequence.firstAttributePos(i);
_L5:
        if (j == 0 || abstractsequence.getNextKind(j) != 35) goto _L2; else goto _L1
_L1:
        if (!type.isInstancePos(abstractsequence, j)) goto _L4; else goto _L3
_L3:
        positionconsumer.writePosition(abstractsequence, j);
_L6:
        j = abstractsequence.nextPos(j);
          goto _L5
_L4:
        if (abstractsequence.getNextKind(j) == 35) goto _L6; else goto _L2
_L2:
    }
}
