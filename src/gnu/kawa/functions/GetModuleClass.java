    ...
    implements Inlineable
{
    ...
    public static Expression getModuleClassURI(Compilation compilation)
    {
        ...
        if (declaration == null)
        {
            declaration = new Declaration(CLASS_RESOURCE_NAME, typeURLPath);
            declaration.setFlag(0x20004800L);
            Object obj;
            if (compilation.immediate)
            {
                ...
                if (!(obj1 instanceof URLPath))
                {
                    obj1 = URLPath.valueOf(((Path) (obj1)).toURL());
                }
                obj = QuoteExp.getInstance(obj1);
            } else
            {
                ApplyExp applyexp = new ApplyExp(getModuleClass, Expression.noExpressions);
                obj = new ApplyExp(maker, new Expression[] {
                    applyexp
                });
            }
            declaration.setValue(((Expression) (obj)));
            compilation.mainLambda.add(null, declaration);
        }
        referenceexp = new ReferenceExp(declaration);
        if (compilation.immediate)
        {
            return referenceexp;
        } else
    ...
    public void compile(ApplyExp applyexp, Compilation compilation, Target target)
    {
        if (this == getModuleUriDummy)
        {
            ...
            if (expression != null)
            {
                BindingInitializer.create(declaration, expression, compilation);
