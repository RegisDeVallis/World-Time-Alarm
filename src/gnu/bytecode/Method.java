    ...
    implements AttrContainer, Member
{
    ...
    Method(ClassType classtype, int i)
    {
        ...
        classtype.methods_count = 1 + classtype.methods_count;
        access_flags = i;
        classfile = classtype;
    }
    ...
    public static String makeSignature(Type atype[], Type type)
    {
        ...
        for (int j = 0; j < i; j++)
        {
            stringbuilder.append(atype[j].getSignature());
        }
    ...
    public void listParameters(StringBuffer stringbuffer)
    {
        ...
        for (int j = 0; j < i; j++)
        {
            ...
            stringbuffer.append(arg_types[j].getName());
        }
    ...
    public void setSignature(String s)
    {
        ...
        do
        {
            int k = Type.signatureLength(s, j);
            if (k < 0)
            {
                if (j < i && s.charAt(j) == ')')
                {
                    arg_types = new Type[stack.size()];
                    for (int l = stack.size(); --l >= 0;)
                    {
                        arg_types[l] = (Type)stack.pop();
                    }
            ...
            stack.push(Type.signatureToType(s, j, k));
            j += k;
        } while (true);
        return_type = Type.signatureToType(s, j + 1, -1 + (i - j));
    }
    ...
    public final void setStaticFlag(boolean flag)
    {
        if (flag)
        {
            access_flags = 8 | access_flags;
            return;
        } else
        {
            access_flags = -9 ^ access_flags;
            return;
        }
    }
